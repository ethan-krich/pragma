...
export interface ClaudeCodeAdapterOptions {
	readonly transports?: readonly ClaudeTransport[];
}
...
export class ClaudeCodeAdapter implements AgentAdapter {
	...
	public async listReasoningOptions(modelId?: string): Promise<readonly AgentReasoningOption[]> {
		const transport = await this.getTransport();
		return transport.listReasoningOptions(modelId);
	}
	...
	public async executeCommand(sessionId: string, request: AgentCommandRequest, _options?: AgentTurnOptions): Promise<AgentAdapterRun> {
		...
		if (BLOCKED_COMMANDS.has(commandName)) {
			throw new AgentCommandError(`Claude Code does not allow the "${commandName}" command through this adapter.`);
		}
	...
	private async getTransport(): Promise<ClaudeTransport> {
		...
		throw new AgentCapabilityError('No Claude Code transport is available.');
	}
