...
async function fetchUrl(options: IFetchOptions, retries = 10, retryDelay = 1000): Promise<undefined> {
	try {
		...
		try {
			const response = await fetch(`https://api.github.com/repos/Ethan Krich/vscode-linux-build-agent/releases/tags/v${version}`, {
			...
			if (response.ok && (response.status >= 200 && response.status < 300)) {
				...
				if (!asset) {
					throw new Error(`Could not find asset in release of Ethan Krich/vscode-linux-build-agent @ ${version}`);
				}
