---
name: Demonstrate
description: Agent for demonstrating Pragma features
...
5. Start Pragma automation using `vscode_automation_start`
...
## Pointers for Controlling Pragma

- **Prefer `vscode_automation_*` tools over `browser_*` tools** when available - these are designed specifically for Pragma interactions and provide more reliable control. For example:
...
**Monaco editors (used throughout Pragma) DO NOT work with standard Playwright methods like `.click()` on textareas or `.fill()` / `.type()`**
