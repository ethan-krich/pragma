...
/// Number of seconds where, if there's no connections to a Pragma server,
...
/// served the latest version of the Pragma server whenever they load the
/// page. The Pragma server prefixes all assets and connections it loads with
/// its version string, so existing clients can continue to get served even
/// while new clients get new Pragma Server versions.
...
/// Gets the release info from the Pragma path prefix, which is in the
...
impl ConnectionManager {
	...
	/// Gets the StartData for the a version of the Pragma server, triggering
