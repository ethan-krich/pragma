...
/*
 * Implements a standalone CLI app that opens Pragma from a remote terminal.
 ...
 *  - In external terminals for WSL this calls Pragma on the Windows side.
 *    The Pragma desktop executable path is passed in env VSCODE_CLIENT_COMMAND.
 */
...
export async function main(desc: ProductDescription, args: string[]): Promise<void> {
	if (!cliPipe && !cliCommand) {
		console.log('Command is only available in WSL or inside a Pragma terminal.');
		return;
	}
...
function fatal(message: string, err: unknown): void {
	console.error('Unable to connect to Pragma server: ' + message);
	console.error(err);
	process.exit(1);
}
